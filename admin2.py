...
# สร้างโฟเดอร์สำหรับเก็บรูปโปรไฟล์ ถ้ายังไม่มี
...
### START: เพิ่ม Path รูป Admin Table View ###
PIC_ADMIN_TABLE_VIEW = "D:\\ส้มตำฟรุ้งฟริ้ง\\picnudee\\admin table.png" # ชื่อไฟล์รูปหน้าโต๊ะแอดมินที่คุณส่งมา
### END: เพิ่ม Path รูป Admin Table View ###
...
### START: เพิ่ม Global รูป Admin Table View ###
global bg_image_admin_table_view
bg_image_admin_table_view = None
### END: เพิ่ม Global รูป Admin Table View ###
...
def admin_panel_page(prev_window):
    ...
    global bg_image_admin_panel
    try:
        # โหลดรูปพื้นหลัง admin.png จาก Path เต็มโดยตรง
        ...
        messagebox.showerror("ข้อผิดพลาด", f"ไม่สามารถโหลดรูปภาพได้: {e}\n{PIC_ADMIN_PANEL}") # ใช้ PIC_ADMIN_PANEL ตรงๆ
        admin_window.destroy()
        root.deiconify()
        return

    # --- ปุ่ม Placeholder (ยังไม่ได้ใส่ command สำหรับ sales และ menu) ---
    btn_font = ("UID SALMON 2019", 50, "bold") # ใช้ Font นี้ตามปุ่มอื่น
    ...
    # *** 2. ปุ่มโต๊ะลูกค้า/ออเดอร์ (เพิ่ม command) ***
    table_order_button = tk.Button(admin_window, text="โต๊ะลูกค้า/คำสั่งซื้อ", font=("UID SALMON 2019", 40, "bold"), # ลดขนาด font เล็กน้อย
                                   bg="#ffffff", fg="#552c1f", bd=0, relief="groove",
                                   command=lambda: admin_table_view_page(admin_window)) # <<< เพิ่ม command
    table_order_button.place(x=160, y=305, width=285 , height=85)

    # 3. ปุ่มเพิ่ม/แก้ไข/ลบ เมนู
    menu_button = tk.Button(admin_window, text="เพิ่ม/แก้ไข/ลบ เมนู", font=("UID SALMON 2019", 40, "bold"), # ลดขนาด font เล็กน้อย
...
### START: หน้าใหม่ (Admin Table View) - แก้ไข: เอาส่วน ! ออก ###
def admin_table_view_page(prev_window):
    ...
    # ตั้งค่า protocol การปิดหน้าต่างใหม่ (ให้กลับไปหน้า Admin Panel)
    admin_table_window.protocol("WM_DELETE_WINDOW", lambda: admin_panel_page(admin_table_window))

    global bg_image_admin_table_view
    try:
        # โหลดรูปพื้นหลัง admin table.jpg
        bg_image_admin_table_pil = Image.open(f"{PIC_ADMIN_TABLE_VIEW}")
        ...
        # กลับไปหน้า Admin Panel ถ้าโหลดรูปไม่ได้
        admin_panel_page(None) # ส่ง None เพราะไม่มีหน้าต่างก่อนหน้า
        return
 
    # --- สร้างปุ่มโต๊ะ ---
    table_coords_admin = [ # ตำแหน่งคร่าวๆ จากรูป admin table.jpg
        (220, 78), (555, 78),
        (220, 288), (555, 285)
    ]
    table_buttons = {} # เก็บ reference ของปุ่ม (ถ้าต้องการใช้)

    for i, (x, y) in enumerate(table_coords_admin):
        table_num = i + 1
        btn = tk.Button(admin_table_window, text=f"โต๊ะ\n{table_num}", font=("Arial", 50, "bold"), # ปรับ Font ให้เหมือนในรูป
                      bg="#ffe0f1", fg="#552c1f", bd=0 , relief="groove",
                      justify=tk.CENTER,
                      command=lambda num=table_num: view_table_order(admin_table_window, num)) # Placeholder command
        btn.place(x=x, y=y, width=165, height=155)
        table_buttons[table_num] = btn

        
    # --- Placeholder: ฟังก์ชันเมื่อกดดูออเดอร์ของโต๊ะ ---
    def view_table_order(current_window, table_number):
        messagebox.showinfo("ดูออเดอร์", f"ดูออเดอร์โต๊ะ {table_number}\n(ยังไม่ได้ทำ)")
        # ในอนาคต หน้านี้จะแสดงรายการอาหารที่โต๊ะนั้นสั่ง

    # --- ปุ่มย้อนกลับ ---
    back_button = tk.Button(admin_table_window, text="ย้อนกลับ", font=("UID SALMON 2019", 45, "bold"),
                             bg="#ffe0f1", fg="#552c1f", bd=0, relief="groove",
                             command=lambda: admin_panel_page(admin_table_window)) # กลับไปหน้า Admin Panel
    back_button.place(x=744, y=468, width=140, height=60) # ตำแหน่งตามรูป

    # ปุ่มผู้พัฒนา (About Me)
    add_about_button(admin_table_window)

### END: หน้าใหม่ (Admin Table View) - แก้ไข ###
