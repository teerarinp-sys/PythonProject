...
### START: เพิ่ม Global รูปใหม่ ###
global bg_image_forgot
bg_image_forgot = None
### END: เพิ่ม Global รูปใหม่ ###
...
### START: หน้าใหม่ (ลืมรหัสผ่าน) ###
def forgot_password_page(prev_window):
    """ฟังก์ชันสำหรับหน้า 'รหัสผ่านใหม่' (หน้าที่ 4)"""
    if prev_window:
        prev_window.destroy()
        
    forgot_window = create_toplevel_window("หนูดีส้มตำฟรุ้งฟริ้ง - รหัสผ่านใหม่")
    
    # ตัวแปรสำหรับเก็บข้อมูลที่ตรวจสอบแล้ว
    verified_contact = tk.StringVar(value="")

    global bg_image_forgot
    try:
        # โหลดจาก Path เต็ม (PIC_FORGOT_PASSWORD) โดยตรง
        ...
        # กลับไปหน้าหลัก (root) ถ้าโหลดไม่สำเร็จ
        root.deiconify() 
        return
    ...
    # 1. ช่องกรอก Email/Phone (อ้างอิงตำแหน่งจากรูป 4.png)
    ...
    # 2. ช่องกรอก รหัสผ่านใหม่ (เริ่มแบบปิด)
    ...
    # 3. ช่องกรอก ยืนยันรหัส (เริ่มแบบปิด)
    ...
    # --- Logic Functions ---
    def handle_check():
        """ตรวจสอบว่า Email/Phone มีในระบบหรือไม่"""
        ...
        if record:
            ...
            # บันทึก contact ที่ตรวจสอบผ่านแล้ว
    ...
    back_button = tk.Button(forgot_window, text="ย้อนกลับ", font=("UID SALMON 2019", 40, "bold"),
                             ...
                             command=lambda: login_page(forgot_window)) # กลับไปหน้า Login
...
def login_page(prev_window=None): # เพิ่ม prev_window
    ...
    # ใช้ create_toplevel_window แทนการสร้างเอง
    login_window = create_toplevel_window("หนูดีส้มตำฟรุ้งฟริ้ง - เข้าสู่ระบบ")
    
    # ตั้งค่า protocol การปิดหน้าต่างใหม่ (ให้กลับไปหน้า main)
    ...
    # *** ปุ่มลืมรหัส (เพิ่มใหม่) ***
    # อ้างอิงตำแหน่งจากรูป 2.jpg ที่คุณส่งมา
    forgot_btn = tk.Button(login_window, text="ลืมรหัส", font=("Arial", 20, "bold"),
                           bg="#daf1ff", fg="#552c1f", bd=0, relief="flat", # สีอาจจะต้องปรับ
                           activebackground="#daf1ff",
                           command=lambda: forgot_password_page(login_window))
    forgot_btn.place(x=638, y=325, width=82, height=35) # ตำแหน่งคร่าวๆ
