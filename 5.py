...
PIC_LOGIN = "2.png"
PIC_REGISTER = "3.png"
PIC_TABLE_SELECT = "D:\\ส้มตำฟรุ้งฟริ้ง\\picnudee\\7.png" # PATH รูปภาพหน้าเลือกโต๊ะ
PIC_PROFILE_VIEW = "D:\\ส้มตำฟรุ้งฟริ้ง\\picnudee\\profile.png" # PATH รูปภาพหน้าโปรไฟล์ (View)
PIC_PROFILE_EDIT = "D:\\ส้มตำฟรุ้งฟริ้ง\\picnudee\\.png" # PATH รูปภาพหน้าโปรไฟล์ (Edit)
PIC_ABOUT = "about.png" 
...
def create_db_table():
    ...
    try:
        ...
        # *** โค้ดส่วนที่เพิ่มผู้ใช้เริ่มต้น 'admin:123' ถูกนำออกแล้ว ***
...
def update_user_profile(old_username, new_data):
    ...
    try:
        ...
        if new_username != old_username:
            ...
            # ต้องเก็บรหัสผ่านเดิมไว้ เพราะไม่ได้แก้ไขในหน้านี้
            cursor.execute("SELECT password FROM user_account WHERE username = ?", (old_username,))
            current_password = cursor.fetchone()[0]

            cursor.execute("""
                UPDATE user_account SET username=?, name=?, surname=?, phone=?, birthday=?, email=? WHERE username=?
            """, (new_username, new_data['name'], new_data['surname'], new_data['phone'], new_data['birthday'], new_data['email'], old_username))
            
            # 3. อัปเดตตาราง user_profile (ต้องลบแล้วสร้างใหม่เนื่องจาก username เป็น PRIMARY KEY)
            # แต่เนื่องจากเราตั้งค่า FOREIGN KEY เป็น CASCADE UPDATE ไม่ได้ เราจึงใช้วิธีลบแล้วสร้างใหม่
            
            # ดึงข้อมูล profile เดิม
            cursor.execute("SELECT score, profile_pic_path FROM user_profile WHERE username = ?", (old_username,))
...
def about_page():
    ...
    try:
        bg_image_about_pil = Image.open(f"D:\\ส้มตำฟรุ้งฟริ้ง\\picnudee\\about.png")
...
def table_selection_page():
    ...
    try:
        ...
        tk.Label(table_window, text="ไม่พบรูปภาพพื้นหลัง (7.jpg)", font=("Arial", 24)).pack(expand=True)
...
# ------------------------------------
def profile_view_page(prev_window):
...
def profile_edit_page(prev_window, user_data):
    ...
    username_var = tk.StringVar(value=user_data['username']) # เพิ่ม Username
    ...
    # Username Entry (ถูกเพิ่มเข้ามา)
    tk.Label(edit_window, font=("UID SALMON 2019", 20), fg="#552c1f", bg="#ffe0f1").place(x=420, y=130)
    username_entry = tk.Entry(edit_window, textvariable=username_var, font=entry_font, bg=bg_color, bd=0)
    username_entry.place(x=520, y=130, width=200, height=30)
    
    # Name Entry (ชื่อ)
    tk.Entry(edit_window, textvariable=name_var, font=entry_font, bg=bg_color, bd=0).place(x=520, y=200, width=150, height=30)
    
    # Surname Entry (นามสกุล)
    tk.Entry(edit_window, textvariable=surname_var, font=entry_font, bg=bg_color, bd=0).place(x=780, y=200, width=150, height=30)
    
    # Phone Entry
    tk.Entry(edit_window, textvariable=phone_var, font=entry_font, bg=bg_color, bd=0).place(x=520, y=270, width=200, height=30)
    
    # Birthday Entry
    tk.Entry(edit_window, textvariable=birthday_var, font=entry_font, bg=bg_color, bd=0).place(x=780, y=270, width=150, height=30)
    
    # Email Entry
    tk.Entry(edit_window, textvariable=email_var, font=entry_font, bg=bg_color, bd=0).place(x=520, y=340, width=300, height=30)
    ...
    change_pic_button.place(x=130, y=380, width=100, height=30)
    
    # ชื่อผู้ใช้ปัจจุบัน (แสดงเป็น Label ด้านล่างซ้าย)
    tk.Label(edit_window, text=f"ชื่อผู้ใช้เดิม: {user_data['username']}", font=entry_font, fg="#552c1f", bg="#fff2f9").place(x=100, y=420)
    
    # คะแนน (ไม่สามารถแก้ไขได้)
    tk.Label(edit_window, text=f"คะแนนของคุณ: {user_data['score']}", font=entry_font, fg="#552c1f", bg="#fff2f9").place(x=490, y=410)
    ...
    def save_changes():
        ...
        if update_user_profile(user_data['username'], new_data):
            ...
            # ต้องส่ง username ที่อาจถูกเปลี่ยนไปให้หน้า view
    ...
    back_button = tk.Button(edit_window, text="ย้อนกลับ", font=("UID SALMON 2019", 16),
                             bg="#ffe0f1", fg="#552c1f", bd=0, relief="flat",
                             command=lambda: profile_view_page(edit_window, user_data)) # กลับไปหน้า 5
    back_button.place(x=350, y=470, width=120, height=40) 

    # ปุ่มบันทึก
    save_button = tk.Button(edit_window, text="บันทึก", font=("UID SALMON 2019", 16, "bold"),
                             bg="#ffe0f1", fg="#552c1f", bd=0, relief="flat",
                             command=save_changes)
    save_button.place(x=500, y=470, width=120, height=40) 
